...
pub fn compute_block_hash(block: &Block) -> Result<StarknetBlockHash> {
    ...
    let sequencer_address = H160::zero();
    ...
    let data = [
        ...
        state_root.0,
        // sequencer address
        StarkHash::from_be_slice(sequencer_address.as_bytes()).unwrap(),
        // block timestamp
    ...
    let block_hash = pedersen_hash_of_array(data.into_iter());
...
fn calculate_transaction_hash_with_signature(tx: &Transaction) -> StarkHash {
    let signature_hash = match &tx.signature {
        None => StarkHash::ZERO,
        Some(signatures) => pedersen_hash_of_array(signatures.iter().map(|e| e.0.to_owned())),
    };

    pedersen_hash(tx.transaction_hash.0, signature_hash)
}
...
fn calculate_event_hash(event: &Event) -> StarkHash {
    let keys_hash = pedersen_hash_of_array(event.keys.iter().map(|key| key.0));
    let data_hash = pedersen_hash_of_array(event.data.iter().map(|data| data.0));

    pedersen_hash_of_array([event.from_address.0, keys_hash, data_hash].into_iter())
}

fn pedersen_hash_of_array<T: Iterator<Item = StarkHash>>(elements: T) -> StarkHash {
    ...
    let (count, hash) = elements.fold((0u64, StarkHash::ZERO), |(count, hash), x| {
        (count.checked_add(1).unwrap(), pedersen_hash(hash, x))
    });
    let count = StarkHash::from_u64(count);
    pedersen_hash(hash, count)
}
...
mod tests {
    ...
    fn test_compute_hash_on_elements() {
        ...
        .unwrap();
        let computed_hash = pedersen_hash_of_array(elements.into_iter());
    ...
    fn test_block_hash() {
        ...
        assert_eq!(block.block_hash, Some(block_hash));
    }
}
